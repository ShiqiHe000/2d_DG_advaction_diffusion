...
SUBROUTINE DG_TIME_DER_COMBINE(T)
    ...
    ALLOCATE(SOLUTION_INT_L(0:TOTAL_CELLS, NUM_OF_EQUATION, 0:LOCAL_ELEM_NUM-1))
    ALLOCATE(SOLUTION_INT_R(0:TOTAL_CELLS, NUM_OF_EQUATION, 0:LOCAL_ELEM_NUM-1))
    
    SOLUTION_INT_L = 0.0D0; SOLUTION_INT_R = 0.0D0
    
    CALL ATTACH_MEMORY(TOTAL_CELLS+1)   ! INDEX START WITH ZERO
...
!    ENDDO
    ...
    CALL ATTACH_MEMORY(TOTAL_CELLS + 1)
