...
SUBROUTINE DISTRIBUTE_ELEM
    ...
    ALLOCATE(X_LOCAL(2, 0:LOCAL_ELEM_NUM-1))
    ALLOCATE(Y_LOCAL(2, 0:LOCAL_ELEM_NUM-1))
    
    IF(RANK /= 0) THEN
        ALLOCATE(X_HILBERT(2, ORIGINAL_ELEM_NUM))
    ...
    CALL MPI_SCATTERV(X_HILBERT, SENDCOUNTS*2, DISPLS, MPI_DOUBLE_PRECISION, &
                        X_LOCAL, LOCAL_ELEM_NUM*2, MPI_DOUBLE_PRECISION, &
                        & 0, MPI_COMM_WORLD, IERROR)
                        
    CALL MPI_SCATTERV(Y_HILBERT, SENDCOUNTS*2, DISPLS, MPI_DOUBLE_PRECISION, &
                        Y_LOCAL, LOCAL_ELEM_NUM*2, MPI_DOUBLE_PRECISION, &
                        & 0, MPI_COMM_WORLD, IERROR)
